/*
FC1 CUTLASS kernel — CUTLASS GemmUniversal with SM100 TMA warp-specialized epilogue.
Shape: [928256,768]×[768,3072]^T + bias + GELU (tanh approx), no residual (beta=0).
Reference for fc1_w3 / cuBLASLt rank-1 GELU+BIAS comparison, same clock64
sentinel @@CYC metric as fc2_cutlass / fc2_w3 / cublaslt_introspect.

Build:  make fc1-cutlass              (fused: GELU + bias)
        make fc1-cutlass-strip        (GEMM-only: no epilogue fusion)
*/
...
/* Problem dimensions — identical to fc1_w3.cu (overridable via -D flags) */
#define SM_COUNT  148
#ifndef M_TOTAL
...
No bias, no GELU — just the matmul.
*/
using FusionOp = cutlass::epilogue::fusion::LinearCombination<
    ElementD, ElementAcc>;
#else
/*
Fused: D = GELU_taylor(acc + bias), beta=0 (no residual).
GELU_taylor = tanh approximation, same family as fc1_w3's gelu_approx and
cuBLASLt's CUBLASLT_EPILOGUE_GELU_BIAS.
*/
...
/*
  ...
  technique and clock domain as cublaslt_introspect.cu / fc1_w3 / fc2_w3,
  so cyc numbers from all harnesses are directly comparable without
...
/* Host GELU (tanh approx) for validation — matches GELU_taylor<float> */
...
int main() {
    ...
    return 1;
#else
    printf("FC1 CUTLASS — GemmUniversal with SM100 TMA warp-specialized epilogue\n");
#ifdef STRIP_EPILOGUE
    printf("  MODE: GEMM-only (STRIP_EPILOGUE — no bias, no GELU)\n");
#else
    printf("  MODE: Fused (GELU + bias, beta=0)\n");
#endif
    ...
    /* Init B: even cols = 0x3C (FP8 1.5), odd cols = 0x38 (FP8 1.0) — matches fc1_w3 */
    {
    ...
    /* Init bias: float(c+1) — matches fc1_w3 (bf16(c+1) promoted to float) */
    {
    ...
    /* Strided checksum — same as fc1_w3 */
    double cksum = 0;
    {
    ...
    /* Spot checks — CUTLASS FP32 epilogue math: bf16(gelu(acc_fp32 + bias_fp32)) */
    int errors = 0;
    for (int spot = 0; spot < 32; spot++) {
        ...
        __nv_bfloat16 expected = __float2bfloat16(gelu_fwd(gemm_val + bias_f));
#endif
