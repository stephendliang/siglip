...
# Batch 10: Warp limit — how does overhead scale with number of active epilogue warps?
BATCH10_EXPERIMENTS=(
    "wlimit_1|-DEPI_WARP_LIMIT=1|0"
    "wlimit_2|-DEPI_WARP_LIMIT=2|0"
    "wlimit_3|-DEPI_WARP_LIMIT=3|0"
)
...
run_batch 10 "WARP LIMIT (scaling with active warps)" "${BATCH10_EXPERIMENTS[@]}"
...
if [ -f "$OUTDIR/results.txt" ] && [ "$DRY_RUN" = "0" ]; then
    ...
    if [ -n "$baseline_ms" ] && [ -n "$strip_all_ms" ]; then
        ...
        for exp in "${BATCH1_EXPERIMENTS[@]}" "${BATCH2_EXPERIMENTS[@]}" "${BATCH3_EXPERIMENTS[@]}" "${BATCH5_EXPERIMENTS[@]}" "${BATCH6_EXPERIMENTS[@]}" "${BATCH8_EXPERIMENTS[@]}" "${BATCH9_EXPERIMENTS[@]}" "${BATCH10_EXPERIMENTS[@]}"; do
            IFS='|' read -r label dflags use_timing <<< "$exp"
